...
Design (settled with the maintainer):
  ...
  * **Least privilege** — the pull is written with a DISTINCT, server-side-only
    ClickHouse credential (``SECURITY_CH_*``, mirroring the scratch writer) that
    is GRANTed only **INSERT + SELECT** on ``security.user_employee_access``.  The
    table and the row policy are **DBA-owned** objects (see the canonical DDL
    below); the app issues NO DDL — refresh is append-only and eviction is the
    table's own TTL, so no CREATE/DELETE/ALTER grant is needed.  The per-request
    freshness read uses the ordinary ``mcp_user`` client, which already holds
    SELECT on this table because the row policy's subquery runs in the querying
    user's context.
...
from app.clickhouse_client import get_client, readonly_settings
...
def build_security_client(settings: Settings) -> Client:
    """Build a one-off ClickHouse client from the SERVER-side security credential.

    Non-cached and used only for the rare pull-and-insert (and the idempotent
    schema bootstrap).  Credentials come from ``security_client_params`` (server
    config), never from the request.  The caller MUST close the returned client.
    """
...
def _write_pull(jti: str, codes: list[str], settings: Settings) -> None:
    ...
    client = build_security_client(settings)
    try:
        ...
        # server-clock value the freshness read compares against).
        client.insert(
            table=_TABLE,
