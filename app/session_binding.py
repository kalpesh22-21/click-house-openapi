...
session id and read their scratch/PII.  To close that gap the token service
stamps a ``sid_hash`` claim inside the (already-signed) JWT at mint time, and
the MCP middleware recomputes the hash of the presented header and compares it
against the claim.

This module is the single source of truth for the wire encoding so the minter
(``app.token_service``) and the verifier (``app.mcp_server``) can never drift:
...
This module has no config dependency so the standalone token service can import
it.
"""
